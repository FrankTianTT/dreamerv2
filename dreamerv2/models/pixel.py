...
class ObsEncoder(nn.Module):
    def __init__(self, input_shape, embedding_size, info):
        ...
        self.convolutions = nn.Sequential(
            ...
            activation(),
            # 32*19*19
            nn.Conv2d(d, 2 * d, k - 1, stride=2),
            activation(),
            # 64*9*9
            nn.Conv2d(2 * d, 4 * d, k - 1, stride=2),
            activation(),
            # 128*4*4
        )
    ...
    def embed_size(self):
        ...
        conv2_shape = conv_out_shape(conv1_shape, 0, self.k - 1, 2)
        conv3_shape = conv_out_shape(conv2_shape, 0, self.k - 1, 2)
...
class ObsDecoder(nn.Module):
    def __init__(self, output_shape, embed_size, info):
        ...
        conv2_shape = conv_out_shape(conv1_shape, 0, k - 1, 2)
        conv3_shape = conv_out_shape(conv2_shape, 0, k - 1, 2)
        ...
        self.decoder = nn.Sequential(
            nn.ConvTranspose2d(4 * d, 2 * d, k - 1, 2),
            activation(),
            nn.ConvTranspose2d(2 * d, d, k - 1, 2),
            activation(),
...
def test_encoder():
    ...
    obs_encoder = ObsEncoder(obs_shape, embedding_size, {'depth': 16, 'kernel': 4, 'activation': nn.ReLU})
...
def test_decoder():
    ...
    obs_decoder = ObsDecoder(obs_shape, embedding_size, {'depth': 16, 'kernel': 4, 'activation': nn.ReLU})
    embed = torch.randn(batch_size, embedding_size)
    obs_dist = obs_decoder(embed)
    print(obs_dist.batch_shape, obs_dist.event_shape)
    print(obs_dist.mean.shape, obs_dist.variance.shape)
