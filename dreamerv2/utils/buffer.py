...
class TransitionBuffer():
    def __init__(
        ...
        print("memory size: ")
        print("obs: {} GB".format(sys.getsizeof(self.observation) / (1024 ** 3)))
        print("action: {} GB".format(sys.getsizeof(self.action) / (1024 ** 3)))
        print("reward: {} GB".format(sys.getsizeof(self.reward) / (1024 ** 3)))
        print("terminal: {} GB".format(sys.getsizeof(self.terminal) / (1024 ** 3)))
    ...
    def add(
        ...
        if obs.dtype == np.float32 and self.obs_type == np.uint8:
    ...
    def _retrieve_batch(self, idxs, n, l):
        ...
        return observation.reshape(l, n, *self.obs_shape), self.action[vec_idxs].reshape(l, n, -1), self.reward[
            vec_idxs].reshape(l, n), self.terminal[vec_idxs].reshape(l, n)
...
class EpisodicBuffer():
    ...
    def sample(self):
        ...
        for ind, episode in enumerate(episode_list):
            obs_batch[:, ind], act_batch[:, ind], rew_batch[:, ind], term_batch[:, ind] = self._sample_seq(episode,
                                                                                                           seq_len)
...
class FluidEpisodicBuffer():
    ...
    def sample(self):
        ...
        for i, idx in enumerate(episode_idx):
            obs_batch[:, i], act_batch[:, i], rew_batch[:, i], term_batch[:, i] = self._sample_seq(self.buffer[idx],
                                                                                                   self.lengths[idx],
                                                                                                   seq_len)
        return obs_batch, act_batch, rew_batch, term_batch
